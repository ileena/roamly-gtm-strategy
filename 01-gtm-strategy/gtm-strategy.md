...
```mermaid
flowchart TD
    A[GOAL: EXPAND existing customer usage] --> B[Existing user organizing a trip with friends]
    C[SUPPORT: Travel community recommendations and trip stories] --> B
    D[OWNED CHANNELS: Targeted email and in-product prompts] --> B
    B --> E[PRIMARY MOTION: PLG / Self-serve group booking]
    E --> F[Organizer starts a shared booking]
    F --> G[Friends join and coordinate]
    G --> H[Participants pay their shares]
    H --> I[Group booking confirmed]
    I --> J[MEASURE: Group-booking completion rate]
    I -. Potential growth to validate .-> K[Referrals and repeat bookings]
    K -.-> B
```
...
## شرح الاستراتيجية بالعربي

- **الهدف:** توسيع استخدام العملاء الحاليين للحجز مع مجموعات، وزيادة الحجوزات الإضافية. اكتساب الأصدقاء المدعوين فائدة ثانوية محتملة.
- **الشريحة والجمهور:** مجموعات أصدقاء صغيرة؛ نخاطب المستخدم الحالي الذي ينظم رحلة فعلية، ثم المشاركين. المنظّم يبدأ لكنه لا يضمن موافقة الآخرين أو دفعهم.
- **القيمة والرسائل:** احجزوا التجربة معًا، كل شخص يدفع حصته، والجميع يشاهد خطة مشتركة. الهدف تقليل جهد متابعة الحجوزات والمدفوعات المنفصلة.
- **طريقة النمو:** المنتج يقود التنسيق والحجز والدفع ذاتيًا. مجتمعات السفر تدعم الاكتشاف والثقة، لكن دورها كمحرّك نمو مستقل لم يثبت بعد.
- **المدخلات الخمسة:** نستهدف فائدة أولية سريعة، مع احتمال تأخر إكمال المجموعة. المشترون هم المشاركون لكن القرار مشترك. لا نفترض عقودًا سنوية؛ ندرس اقتصاديات الحجز. المسار ذاتي مع دعم للاستثناءات. الدعوات والتوصيات فرص نمو تحتاج اختبارًا.
- **القنوات:** رسائل مناسبة داخل المنتج وبريد مستهدف، ثم دعوات واضحة للأصدقاء. نختبر توصيات مجتمعات السفر ونقيس الحجوزات الناتجة، لا الزيارات فقط.
- **التجربة الأولى:** مدن ومضيفون جاهزون للمجموعات، مع وضوح مهلة الدفع والإلغاء والاسترجاع. تتعاون فرق المنتج والنمو والعمليات والدعم والمدفوعات والتحليلات على الجاهزية.
